...
def klientu_tabula():
    ...
    tree.heading('Instruments_id', text='Instruments_id')

    for i in cursor.execute('SELECT * FROM klients'):
        tree.insert('', 'end', text="1", values=i)
    
    tree.grid()
    frame_search_klient = LabelFrame(main_tk, text='Add user')
    ...
    Button(frame_search_klient, text='Meklēt Klientu', command=search_button_kliens).grid()
    Button(main_tk, text='Delete', command=delete_klients).grid()


def search_button_kliens():


    if search_klients_entry.get() == '':
        searched_uzvards = cursor.execute('SELECT * FROM klients')
    else:
        searched_uzvards = cursor.execute(f'SELECT * FROM klients WHERE uzvards LIKE "{search_klients_entry.get()}"')
    columns = ('klientu ID',
    "Vārds", 
    "Uzvārds", 
    "Personas kods", 
...
def saglabasana_datne():
    ...
    for i in max(pedeja_id):
        try:
            ...
            pass
    try:
        sqlite_connection.execute(f'INSERT INTO klients VALUES ({pedeja_id+1},"{entry_vards.get()}", "{entry_uzvards.get()}", "{entry_personas_kods.get()}", "{entry_telefona_numurs.get()}","{entry_telefona_kods.get()}", "{entry_sakuma_datums.get()}", "{entry_beiguma_datums.get()}", "{entry_instruments_id}")')
        sqlite_connection.commit()
